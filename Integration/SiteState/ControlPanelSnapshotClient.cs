...
public enum ControlPanelSnapshotFailure
{
    None,
    Network,
    Timeout,
...
public sealed record ControlPanelSnapshotClientResult(
    byte[]? Body,
    ControlPanelSnapshotFailure Failure,
    TimeSpan? RetryAfter = null)
{
    public bool Succeeded => Failure == ControlPanelSnapshotFailure.None;

    public bool IsRetryable => Failure is
        ControlPanelSnapshotFailure.Network or
        ControlPanelSnapshotFailure.Timeout or
        ControlPanelSnapshotFailure.Throttled or
        ControlPanelSnapshotFailure.ServerError;

    public static ControlPanelSnapshotClientResult Success(byte[] body) =>
        new(body, ControlPanelSnapshotFailure.None);

    public static ControlPanelSnapshotClientResult Failed(
        ControlPanelSnapshotFailure failure,
        TimeSpan? retryAfter = null) =>
        new(null, failure, retryAfter);
}

public interface IControlPanelSnapshotClient
{
    Task<ControlPanelSnapshotClientResult> GetSnapshotAsync(
        CancellationToken cancellationToken);
}

public sealed class ControlPanelSnapshotClient : IControlPanelSnapshotClient
{
    ...
    public ControlPanelSnapshotClient(
    ...
    public async Task<ControlPanelSnapshotClientResult> GetSnapshotAsync(
        CancellationToken cancellationToken)
    {
        using var timeout = CancellationTokenSource.CreateLinkedTokenSource(
            cancellationToken);
        timeout.CancelAfter(TimeSpan.FromSeconds(
            _options.SnapshotHttpTimeoutSeconds));

        using var request = new HttpRequestMessage(
            HttpMethod.Get,
            $"api/v1/sites/{_options.SiteId}/snapshot");
        ...
        {
            return ControlPanelSnapshotClientResult.Failed(
        ...
        using (response)
        {
            ...
            if (response.Content.Headers.ContentLength is >
                SiteStateReconciliationOptions.ResponseSizeLimitBytes)
            {
                return ControlPanelSnapshotClientResult.Failed(
    ...
    private ControlPanelSnapshotClientResult ClassifyNonSuccess(
        HttpResponseMessage response) =>
        response.StatusCode switch
        {
            HttpStatusCode.Unauthorized => ControlPanelSnapshotClientResult.Failed(
                ControlPanelSnapshotFailure.Unauthorized),
            HttpStatusCode.Forbidden => ControlPanelSnapshotClientResult.Failed(
                ControlPanelSnapshotFailure.Forbidden),
            HttpStatusCode.NotFound => ControlPanelSnapshotClientResult.Failed(
            ...
            >= HttpStatusCode.InternalServerError =>
                ControlPanelSnapshotClientResult.Failed(
                    ControlPanelSnapshotFailure.ServerError),
            _ => ControlPanelSnapshotClientResult.Failed(
                ControlPanelSnapshotFailure.Protocol)
        };
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            var remaining =
                SiteStateReconciliationOptions.ResponseSizeLimitBytes - total + 1;
            ...
            total += read;
            if (total > SiteStateReconciliationOptions.ResponseSizeLimitBytes)
            {
